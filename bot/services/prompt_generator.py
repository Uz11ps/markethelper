...
class PromptGeneratorService:
    ...
    async def generate_prompt_from_images(
        ...
        try:
            ...
            # Добавляем четкую инструкцию с требованием JSON формата
            content.append({
                "type": "text",
                "text": (
                    "Ты получил на вход:\n"
                    f"1. {len(product_image_urls)} фотографий СВОЕГО ТОВАРА (разные ракурсы, детали)\n"
                    f"2. {len(reference_image_urls)} КАРТОЧЕК ТОВАРА (референсы), которые нравятся пользователю\n\n"
                    "Твоя задача: проанализировать эти изображения и создать 3 уникальные концепции карточек товара.\n\n"
                    "КРИТИЧЕСКИ ВАЖНО:\n"
                    "- Товар должен быть ТОЧНО ТАКИМ ЖЕ, как на фотографиях (форма, цвет, размер, детали)\n"
                    "- Стиль, композиция, фон, цвета должны быть взяты из референсных карточек\n"
                    "- Результат: карточка товара с ТВОИМ товаром в стиле референсов\n\n"
                    "ОТВЕТ ДОЛЖЕН БЫТЬ ТОЛЬКО JSON МАССИВОМ из 3 объектов, начинаться с [ и заканчиваться ]. "
                    "НЕ добавляй текст до или после JSON, НЕ используй markdown блоки."
                )
            })
            ...
            content.append({
                "type": "text",
                "text": f"\n\n**ФОТОГРАФИИ СВОЕГО ТОВАРА ({len(product_image_urls)} шт.):**"
            })
            ...
            for i, url in enumerate(product_image_urls, 1):
                ...
                content.append({
                    "type": "text",
                    "text": f"Фото товара #{i} - проанализируй цвет, форму, размер, материалы, детали"
                })

            # Добавляем референсы
            content.append({
                "type": "text",
                "text": f"\n\n**РЕФЕРЕНСНЫЕ КАРТОЧКИ ТОВАРА ({len(reference_image_urls)} шт.):**"
            })
            ...
            for i, url in enumerate(reference_image_urls, 1):
                ...
                content.append({
                    "type": "text",
                    "text": f"Референс #{i} - проанализируй стиль, композицию, фон, цвета, расположение элементов"
                })
            
            # Добавляем финальное напоминание о формате
            content.append({
                "type": "text",
                "text": (
                    "\n\nПОМНИ: Твой ответ должен быть ТОЛЬКО JSON массивом из 3 объектов. "
                    "Начинай ответ с символа [ и заканчивай символом ]. "
                    "НЕ добавляй текст, комментарии или markdown блоки."
                )
            })
            ...
            if json_start_array != -1 and json_end_array > json_start_array:
                ...
                if json_match:
                    ...
                    # Проверяем, не является ли ответ ошибкой о невозможности анализа изображений
                    if "unable to analyze images" in answer.lower() or "cannot analyze" in answer.lower():
                        logger.error(f"GPT сообщил, что не может анализировать изображения. Ответ: {answer[:500]}")
                        raise ValueError(
                            "GPT не может анализировать изображения. "
                            "Проверьте, что используется модель GPT-4o с поддержкой Vision. "
                            "Если проблема повторяется, обратитесь к администратору."
                        )
                    logger.error(f"Не найден JSON в ответе GPT. Полный ответ: {answer[:500]}")
                    raise ValueError(
                        f"GPT вернул ответ не в формате JSON. "
                        f"Ответ начинается с: {answer[:200]}...\n\n"
